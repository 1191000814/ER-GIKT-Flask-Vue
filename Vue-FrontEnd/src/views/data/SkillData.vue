<template>
  <div style="padding: 10px">
    ...
    <el-table :data="tableData" border style="width: 100%" size="small">
      <el-table-column fixed prop="id" label="知识点ID" sortable></el-table-column>
  ...
  methods: {
    ...
    handleSave: function () {
      if (this.formData.id) { // 参数带有id的，是更新（update）操作
        ...
        request.post("/skill/update", this.formData).then(res => {
          console.log(res)
          if (res.code === 0)
            this.$message({
        ...
        request.post("/skill/add", this.formData).then(res => {
          if (res.code === 0)
            this.$message({
